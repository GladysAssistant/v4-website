...
| --- | --- | --- | --- |
| Connectez votre compte Spotify | `section` | Non | Cette intégration pilote vos appareils Spotify Connect via l'API Web de Spotify. Un compte Spotify Premium est nécessaire pour contrôler la lecture. Créez d'abord une application Spotify gratuite, collez son Client ID et son Client Secret ci-dessous, puis cliquez sur « Se connecter avec Spotify ». |
...
| Le navigateur n'a pas pu revenir à Gladys ? | `section` | Non | Spotify n'accepte qu'une adresse de redirection sur 127.0.0.1, qui désigne la machine où tourne votre navigateur. Si vous accédez à Gladys par l'adresse IP d'un serveur, le retour aboutit à une page blanche ou d'erreur : c'est normal, et l'autorisation a bien réussi. Copiez l'adresse complète depuis la barre d'adresse de votre navigateur, collez-la ci-dessous, enregistrez, puis cliquez sur « Terminer la connexion ». |
...
2. Cliquez sur **Installer**. Gladys télécharge l'image Docker (`ghcr.io/william-de71/gladys-spotify:1.0.0`), la démarre dans un bac à sable isolé du cœur, et génère l'interface de l'intégration (appareils, découverte et configuration).
