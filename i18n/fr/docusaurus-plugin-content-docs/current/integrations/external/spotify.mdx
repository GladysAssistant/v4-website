...
description: "Contrôlez vos appareils Spotify Connect et lancez vos playlists, morceaux récents ou favoris. Gratuite, open source, installable en un clic."
...
| Se connecter avec Spotify | `oauth2` | Non | Une fois le Client ID et le Client Secret enregistrés, cliquez sur Se connecter pour autoriser Gladys sur votre compte Spotify. Si Gladys signale plus tard des autorisations manquantes après une mise à jour, cliquez à nouveau dessus : vos appareils et vos scènes Gladys seront conservés. |
...
2. Cliquez sur **Installer**. Gladys télécharge l'image Docker (`ghcr.io/william-de71/gladys-spotify:1.2.0`), la démarre dans un bac à sable isolé du cœur, et génère l'interface de l'intégration (appareils, découverte et configuration).
...
Spotify nécessite Gladys `>=4.86.0`. Le catalogue dans Gladys se rafraîchit toutes les heures : une nouvelle version est donc disponible au plus tard une heure après sa sortie.
