...
| --- | --- | --- | --- |
| Diaporama 1 — connecter Immich | `section` | Non | Créez une clé API Immich avec les permissions album.read, asset.read, asset.view et memory.read. Gladys stocke cette clé de manière sécurisée ; elle n’est jamais envoyée au navigateur du dashboard. |
...
| UUID des albums | `string` | Non | Requis uniquement pour la source Album. Saisissez un ou plusieurs UUID séparés par des virgules ou des retours à la ligne ; utilisez « Lister les albums Immich » ci-dessous pour les copier. Laissez vide pour Souvenirs. |
...
| Afficher la légende Immich | `boolean` | Non | Ajoute la description, le lieu et la date de prise de vue Immich lorsqu’ils sont disponibles, dans un bandeau lisible au bas de chaque image. |
| Diaporama 2 — connecter Immich | `section` | Non | Optionnel. Configurez ce diaporama indépendant pour afficher un autre album, une autre source de souvenirs ou un autre serveur Immich sur une seconde caméra Gladys. |
| URL du serveur Immich | `string` | Non | Serveur utilisé par le second diaporama. Il peut être identique à celui du diaporama 1 ou pointer vers une autre instance Immich. |
| Clé API Immich | `secret` | Non |  |
| Source des photos | `select` | Non | Choisissez un album précis ou les souvenirs Immich de cette date les années précédentes pour la seconde caméra du dashboard. |
| UUID des albums | `string` | Non | Requis uniquement lorsque la seconde source est Album. Saisissez un ou plusieurs UUID séparés par des virgules ou des retours à la ligne. Laissez vide pour Souvenirs. |
| Intervalle entre les photos (secondes) | `number` | Non | Durée d’affichage de chaque image du second diaporama sur sa caméra du dashboard. |
| Intervalle d’actualisation de la source (secondes) | `number` | Non | Fréquence à laquelle le second diaporama redemande son album ou les souvenirs du jour à Immich. |
| Nombre maximal d’images | `number` | Non | Limite la source du second diaporama pour préserver la réactivité. |
| Mélanger les images | `boolean` | Non | Mélange uniquement les images affichées par le second diaporama. |
| Afficher la légende Immich | `boolean` | Non | Ajoute la description, le lieu et la date de prise de vue Immich aux images du second diaporama lorsqu’ils sont disponibles. |
...
2. Cliquez sur **Installer**. Gladys télécharge l'image Docker (`ghcr.io/gboulvin/gladys-immich:0.1.8`), la démarre dans un bac à sable isolé du cœur, et génère l'interface de l'intégration (appareils, découverte et configuration).
...
Immich Slideshow nécessite Gladys `>=4.86.0`. Le catalogue dans Gladys se rafraîchit toutes les heures : une nouvelle version est donc disponible au plus tard une heure après sa sortie.
