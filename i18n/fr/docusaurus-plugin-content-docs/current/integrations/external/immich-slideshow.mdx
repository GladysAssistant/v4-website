...
| --- | --- | --- | --- |
| Connecter Immich | `section` | Non | Créez une clé API Immich avec les permissions album.read, asset.read, asset.view et memory.read. Gladys stocke cette clé de manière sécurisée ; elle n’est jamais envoyée au navigateur du dashboard. |
...
2. Cliquez sur **Installer**. Gladys télécharge l'image Docker (`ghcr.io/gboulvin/gladys-immich:0.1.7`), la démarre dans un bac à sable isolé du cœur, et génère l'interface de l'intégration (appareils, découverte et configuration).
