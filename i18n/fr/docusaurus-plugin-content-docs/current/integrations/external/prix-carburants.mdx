...
| Type de carburant | `multi_select` | Oui | Un appareil est proposé par station ET par carburant sélectionné, ce qui permet de suivre plusieurs carburants dans la même station. Une station n'apparaît que pour les carburants qu'elle vend vraiment : beaucoup de stations (TotalEnergies notamment) ont remplacé le SP95 par le E10 (SP95-E10), cochez donc E10 si le SP95 de votre station reste introuvable. |
...
2. Cliquez sur **Installer**. Gladys télécharge l'image Docker (`ghcr.io/prohand/gladys-prixcarburants:1.0.5`), la démarre dans un bac à sable isolé du cœur, et génère l'interface de l'intégration (appareils, découverte et configuration).
...
Prix carburants nécessite Gladys `>=4.86.0`. Le catalogue dans Gladys se rafraîchit toutes les heures : une nouvelle version est donc disponible au plus tard une heure après sa sortie.
