...
| Durée de la recherche (secondes) | `number` | Non | Durée d'écoute des annonces mDNS de l'Apple TV. À augmenter sur un réseau chargé ou lent. |
| Adresses IPv4 manuelles | `string` | Non | Facultatif, séparées par des virgules. S'ajoutent aux adresses annoncées à Gladys, jamais à leur place : ce champ ne permet pas de limiter la recherche à une seule Apple TV. Utile si les annonces automatiques n'atteignent pas Gladys mais que le trafic direct sort toujours sur le réseau local de l'Apple TV, comme avec certaines installations Docker de bureau. Une adresse seule ne permet pas la découverte entre VLANs routés. |
...
2. Cliquez sur **Installer**. Gladys télécharge l'image Docker (`ghcr.io/valentinhttr/gladys-apple-tv:1.2.1`), la démarre dans un bac à sable isolé du cœur, et génère l'interface de l'intégration (appareils, découverte et configuration).
...
Apple TV nécessite Gladys `>=5.0.0`. Le catalogue dans Gladys se rafraîchit toutes les heures : une nouvelle version est donc disponible au plus tard une heure après sa sortie.
