...
| Une notification de scène n'est jamais arrivée        | Elle a été envoyée pendant une coupure du démon et a attendu plus de 5 minutes, ou plus de 20 messages se sont accumulés : les logs de l'intégration le disent.                                                                                                                                                       |
| « Connexion au démon Olvid perdue » de temps en temps | Le conteneur du démon s'est arrêté — souvent parce que Gladys lui-même a redémarré. L'intégration s'en aperçoit tout de suite, le relance si besoin et se reconnecte en quelques secondes ; un démon déjà en marche n'est jamais redémarré. La raison d'un arrêt isolé est dans les logs du conteneur `olvid-daemon`. |
...
2. Cliquez sur **Installer**. Gladys télécharge l'image Docker (`ghcr.io/prohand/gladys-olvid:1.0.4`), la démarre dans un bac à sable isolé du cœur, et génère l'interface de l'intégration (appareils, découverte et configuration).
