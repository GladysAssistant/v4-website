...
Write the following text in the `gladys-compose.yml` file.
```yaml
services:
  gladys:
    ...
    volumes:
      ...
      # Host system bus: reboot/shutdown from the System settings, and the
      # prerequisite for Bluetooth (Matter BLE pairing, Bluetooth sensors)
