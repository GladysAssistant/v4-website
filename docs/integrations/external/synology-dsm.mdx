...
| Date format | `select` | No | Format used for the backup dates published to Gladys. Every format except ISO 8601 shows the local time of the integration container. |
...
2. Click **Install**. Gladys pulls the Docker image (`ghcr.io/prohand/gladys-synology:1.0.4`), starts it in a sandbox isolated from the core, and generates the integration's interface (devices, discovery and configuration).
