...
2. Click **Install**. Gladys pulls the Docker image (`ghcr.io/callemand/gladys-melcloud-home:1.4.0`), starts it in a sandbox isolated from the core, and generates the integration's interface (devices, discovery and configuration).
...
MELCloud Home requires Gladys `>=4.86.0`. The catalog inside Gladys refreshes every hour, so a new version becomes available at most one hour after its release.
