...
2. Click **Install**. Gladys pulls the Docker image (`ghcr.io/prohand/gladys-uvindex:1.0.5`), starts it in a sandbox isolated from the core, and generates the integration's interface (devices, discovery and configuration).
...
Indice UV requires Gladys `>=4.86.0`. The catalog inside Gladys refreshes every hour, so a new version becomes available at most one hour after its release.
