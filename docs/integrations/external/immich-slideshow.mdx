...
| --- | --- | --- | --- |
| Connect Immich | `section` | No | Create an Immich API key with album.read, asset.read, asset.view, and memory.read permissions. The key is securely stored by Gladys and is never sent to the dashboard browser. |
...
2. Click **Install**. Gladys pulls the Docker image (`ghcr.io/gboulvin/gladys-immich:0.1.7`), starts it in a sandbox isolated from the core, and generates the integration's interface (devices, discovery and configuration).
