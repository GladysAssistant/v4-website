...
| A scene notification never arrived                 | It was sent while the daemon was down and waited more than 5 minutes, or more than 20 messages piled up: the integration logs say so.                                                                                                                                                              |
| "Connection to the Olvid daemon lost" now and then | The daemon container stopped — most often because Gladys itself restarted. The integration notices at once, starts it back up when needed and reconnects within seconds; a daemon that is already running is never restarted. Why a one-off stop happened is in the `olvid-daemon` container logs. |
...
2. Click **Install**. Gladys pulls the Docker image (`ghcr.io/prohand/gladys-olvid:1.0.4`), starts it in a sandbox isolated from the core, and generates the integration's interface (devices, discovery and configuration).
