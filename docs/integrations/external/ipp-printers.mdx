...
| --- | --- | --- | --- |
| How it works | `section` | No | Printers advertising IPP (any AirPrint-capable printer) are discovered automatically on your network. You can also list printers manually below — useful when mDNS does not reach them. Each printer exposes one sensor per ink or toner cartridge, plus its state. Printers that announce no level over IPP are asked over SNMP instead, the way CUPS reads them. |
...
2. Click **Install**. Gladys pulls the Docker image (`ghcr.io/guim31/gladys-ipp:1.0.10`), starts it in a sandbox isolated from the core, and generates the integration's interface (devices, discovery and configuration).
...
IPP Printers requires Gladys `>=4.86.0`. The catalog inside Gladys refreshes every hour, so a new version becomes available at most one hour after its release.
