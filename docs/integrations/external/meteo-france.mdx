...
2. Click **Install**. Gladys pulls the Docker image (`ghcr.io/william-de71/gladys-meteo-france:1.2.0`), starts it in a sandbox isolated from the core, and generates the integration's interface (devices, discovery and configuration).
...
Météo France requires Gladys `>=5.0.0`. The catalog inside Gladys refreshes every hour, so a new version becomes available at most one hour after its release.
