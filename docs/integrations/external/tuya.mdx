...
sidebar_label: "Tuya"
description: "Tuya / Smart Life sockets, meters, ACs, heaters, cameras, doorbells, feeders, vacuums (cloud + LAN). Free, open source, installable in one click."
...
2. Click **Install**. Gladys pulls the Docker image (`ghcr.io/terdious/gladys-tuya:1.15.0`), starts it in a sandbox isolated from the core, and generates the integration's interface (devices, discovery and configuration).
...
Tuya requires Gladys `>=4.86.1`. The catalog inside Gladys refreshes every hour, so a new version becomes available at most one hour after its release.
