...
| Pause automatic capture below (%) | `number` | No | Below this level the automatic refresh stops. Opening the camera widget or a doorbell press still captures an image. This level is also the ceiling of the one below: a "stop every capture" set higher is lowered to it, so that the on-demand band does not vanish. |
| Stop every capture below (%) | `number` | No | Below this level no image is captured at all, not even on request. Motion detection stops here too, and so does the privacy mode: the camera is only woken every 15 minutes for its battery level, which is what tells when it has recharged. Capture resumes a few points above this level, so a camera sitting right on it does not flip back and forth. |
...
2. Click **Install**. Gladys pulls the Docker image (`ghcr.io/william-de71/gladys-tapo:1.2.5`), starts it in a sandbox isolated from the core, and generates the integration's interface (devices, discovery and configuration).
