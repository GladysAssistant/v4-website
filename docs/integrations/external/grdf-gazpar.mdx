...
| GRDF account password | `secret` | Yes | Never sent back to your browser once saved, and only ever transmitted to GRDF. Like every Gladys setting, it is stored as-is in your server's database. |
...
2. Click **Install**. Gladys pulls the Docker image (`ghcr.io/philippema/gladys-grdf:1.1.6`), starts it in a sandbox isolated from the core, and generates the integration's interface (devices, discovery and configuration).
...
GRDF Gazpar requires Gladys `>=4.86.0`. The catalog inside Gladys refreshes every hour, so a new version becomes available at most one hour after its release.
