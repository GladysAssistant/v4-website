...
| Discovery duration (seconds) | `number` | No | How long Gladys listens for the Apple TV mDNS announcements. Increase it on a busy or slow network. |
| Manual IPv4 addresses | `string` | No | Optional, comma separated. Added to the addresses Gladys announces, never in place of them: this field cannot be used to restrict the scan to one Apple TV. Useful when automatic announcements do not reach Gladys but direct traffic still exits on the Apple TV's local subnet, as with some desktop Docker setups. An address alone does not enable discovery across routed VLANs. |
...
2. Click **Install**. Gladys pulls the Docker image (`ghcr.io/valentinhttr/gladys-apple-tv:1.2.1`), starts it in a sandbox isolated from the core, and generates the integration's interface (devices, discovery and configuration).
...
Apple TV requires Gladys `>=5.0.0`. The catalog inside Gladys refreshes every hour, so a new version becomes available at most one hour after its release.
