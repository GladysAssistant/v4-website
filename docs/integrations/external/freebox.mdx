...
| --- | --- | --- | --- |
| Connect your Freebox | `section` | No | This integration talks to your Freebox Server over your local network. To pair it, click "Pair with the Freebox" below, then physically confirm the authorization on your Freebox Server LCD screen (press the right arrow on the box). The pairing is done only once. |
...
2. Click **Install**. Gladys pulls the Docker image (`ghcr.io/william-de71/gladys-freebox:1.0.7`), starts it in a sandbox isolated from the core, and generates the integration's interface (devices, discovery and configuration).
